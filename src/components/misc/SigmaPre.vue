<template>
  <div class="sigma-text-wrapper">
    <div ref="contentContainer" class="sigma-text-content-host">
      <slot></slot>
    </div>
  </div>
</template>
...
const applyAnimation = async () => {
  if (!contentContainer.value) {
    console.warn("Content container not found for animation.")
    return
  }

  const preElement = contentContainer.value.querySelector("pre")

  if (!preElement) {
    console.warn(
      "SigmaText component expects a <pre> element in its slot to animate.",
    )
    return
  }
