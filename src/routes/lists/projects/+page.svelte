<script lang="ts">
    ...
    import { data } from "$lib/gallery/data/projects.svelte";
    import { filters } from "$lib/gallery/filters/projects.svelte";
    import Seo from "../../../components/misc/Seo.svelte";

    const project_data = data();
    const projects_filters = filters(project_data.projects);
...
<div class="gallery-wrapper">
    ...
    {#if project_data.is_loading()}
        <div class="loading">
            {$_("indicators.loading", {
                values: {
                    progress: project_data.dl_progress(),
                },
            })}
        </div>
    {/if}

    {#if project_data.fetch_err()}
        <div class="error">
            {$_("indicators.list-load-error", {
                values: {
                    err: project_data.fetch_err(),
                },
            })}
        </div>
    {/if}
    ...
    <div class="filters">
        <input
            bind:value={projects_filters.searchFilters.name}
            type="text"
        ...
        <div
            bind:this={tagsDropdownRef}
            class="multi-select"
        >
            <!-- svelte-ignore event_directive_deprecated -->
            <div
                class="dropdown"
                class:open={projects_filters.dropdowns.tags}
                tabindex="0"
                on:click|stopPropagation={() => projects_filters.openDropdown("tags")}
                on:blur|capture={() => projects_filters.delayedCloseDropdown("tags")}
            >
                <div class="dropdown-content">
                    {#if projects_filters.searchFilters.tags.length === 0}
                        <span class="placeholder">{$_("gallery.filters.select-tags")}</span>
                    {:else}
                        <div class="selected-tags">
                            <div class="tags-wrapper">
                                {#each projects_filters.searchFilters.tags as tag}
                                    <span
                                        class="tag-button"
                                        on:click|stopPropagation={() =>
                                            projects_filters.toggleFilterTag(tag)}
                                    >
                ...
                {#if projects_filters.dropdowns.tags}
                    <ul class="dropdown-list">
                        {#if projects_filters.filteredTagCounts.length > 0}
                            {#each projects_filters.filteredTagCounts() as [tag, count]}
                                <!-- svelte-ignore a11y_no_noninteractive_element_interactions -->
                                <li
                                    class:selected={projects_filters.searchFilters.tags.includes(
                                        tag,
                                    )}
                                    on:click|stopPropagation={() =>
                                        projects_filters.toggleFilterTag(tag)}
                                >
                                    <input
                                        type="checkbox"
                                        checked={projects_filters.searchFilters.tags.includes(tag)}
                                        readonly
                                    />
    ...
    {#if !project_data.is_loading() && !project_data.fetch_err()}
        <div class="gallery">
            {#each projects_filters.filteredProjects() as project}
                <div class="card">
            ...
            {#if projects_filters.allTags.length !== 0}
                {#if projects_filters.filteredProjects.length === 0}
