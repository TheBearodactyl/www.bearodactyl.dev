...
export const games_display = display();
export function use_games_display() {
    onMount(() => {
        games_display.reset_idle_timer();

        if (browser) {
            document.addEventListener("keydown", games_display.reset_idle_timer);
            window.addEventListener("mousemove", games_display.reset_idle_timer);
            window.addEventListener("keydown", games_display.reset_idle_timer);
            window.addEventListener("scroll", games_display.reset_idle_timer);
            window.addEventListener("click", games_display.reset_idle_timer);

            document.addEventListener("keydown", (event) => {
                if (event.key === "Escape") {
                    games_display.close_card();
                }
            });
        }
    });
