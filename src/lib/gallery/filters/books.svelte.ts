...
export function filters(all_books: () => Book[]) {
    const storage = create_book_filters_store();
    ...
    function apply_filter(book: Book): boolean {
    ...
    const filtered_books = $derived(() => {
        ...
        return books.filter(apply_filter);
    });
    ...
    const filtered_tag_counts = $derived(() => {
        ...
        const books = all_books().filter((book) => {
    ...
    const filtered_genre_counts = $derived(() => {
        ...
        const books = all_books().filter((book: Book) => {
            const { genres, rating, status } = storage.filters;
    ...
    return {
        ...
        set search_filters(value: BookFilters) {
